...
function WizardContent() {
  const { currentStep, totalSteps, nextStep, prevStep, isLoading, wizardData } =
    useWizard();
  ...
  const validateScheduleConfiguration = () => {
    ...
    const hasMonthly =
      scheduleConfig.monthly && scheduleConfig.monthly.length > 0;
    const hasCalendar =
      scheduleConfig.calendar && scheduleConfig.calendar.length > 0;
    ...
    if (currentStep === 4) {
      if (!validateScheduleConfiguration()) {
        toast({
          ...
          description:
            "Please configure at least one schedule (Daily, Weekly, Monthly, or Calendar) before proceeding.",
          variant: "destructive",
        });
        return;
      }
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return <ManualPostStep3 />; // Configure Post for Each Platform
      case 3:
        return <ManualPostStep4 />; // Link Configuration
      case 4:
        return <ManualPostStep5 />; // Schedule Configuration
      case 5:
        return <ManualPostStep6 />; // Schedule Summary
      case 6:
        return <ManualPostStep7 />; // Final Step: Schedule Name
      default:
        return <ManualPostStep1 />;
    }
  };
  ...
  return (
    <div className="page-content-full">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-standard mb-2">
          Create Manual Post
        </h1>
        <p className="text-muted">
          Create and schedule your social media content manually
        </p>
      </div>
